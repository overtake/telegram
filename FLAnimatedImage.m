...
- (instancetype)initWithAnimatedGIFData:(NSData *)data imageDrawingBlock:(NSImage *(^)(NSImage *))imageDrawingBlock
{
    ...
    if (self) {
        ...
        for (size_t i = 0; i < imageCount; i++) {
            ...
            if (frameImageRef) {
                NSImage *frameImage = [[NSImage alloc] initWithCGImage:frameImageRef size:NSMakeSize(CGImageGetWidth(frameImageRef), CGImageGetHeight(frameImageRef))];
...
- (NSImage *)imageLazilyCachedAtIndex:(NSUInteger)index
{
    ...
    if ([tryImage isKindOfClass:[NSImage class]]) {
        image = tryImage;
    }
...
#endif
                NSImage *image = [weakSelf predrawnImageAtIndex:i];
#if DEBUG
...
- (NSImage *)predrawnImageAtIndex:(NSUInteger)index
{
...
// On success, the returned object is a new `NSImage` instance with the same content as the one passed in.
...
+ (NSImage *)predrawnImageFromImage:(NSImage *)imageToPredraw
{
    ...
    NSImage *predrawnImage = [[NSImage alloc] initWithCGImage:predrawnImageRef size:NSMakeSize(CGImageGetWidth(predrawnImageRef), CGImageGetHeight(predrawnImageRef))];
...
- (NSString *)description
{
    ...
    description = [description stringByAppendingFormat:@" size=%@", NSStringFromSize(self.size)];
