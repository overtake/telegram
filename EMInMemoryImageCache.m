...
- (instancetype)initWithKey:(id<NSCopying>)key image:(NSImage *)image accessIndex:(NSUInteger)accessIndex
{
    self = [super init];
    if (self != nil)
    {
...
- (void)_addResidentImage:(NSImage *)image forKey:(id<NSCopying>)key
{
    NSUInteger imageSize = (NSUInteger)(image.size.width * image.size.height * 1 * 4);
    if (_residentImagesTotalSize + imageSize > _maxResidentSize)
    {
        ...
        for (NSInteger i = ((NSInteger)sortedImages.count) - 1; i >= 0 && removedSize < sizeToRemove; i--)
        {
            ...
            NSInteger currentImageSize = (NSInteger)(currentImage.image.size.width * currentImage.image.size.height * 1 * 4);
