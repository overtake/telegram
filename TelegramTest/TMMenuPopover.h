...
@class TMMenuPopover;
@class TMMenuPopoverBackgroundView;

// Defines the different types of behavior of a TMMenuPopover.
//
// TMMenuPopoverBehaviorApplicationDefined - The application decides when the
//                                        popover should open and close, by
//                                        doing so manually.
// TMMenuPopoverBehaviorTransient          - If there is a mouse down anywhere
//                                        other than in the popover the popover
//                                        is closed. It is also closed if the
//                                        app or parent window lose focus or
//                                        `esc` is pressed.
// TMMenuPopoverBehaviorSemiTransient      - Closes the popover only if there is a
//                                        mouse up within the popover's parent
//                                        window or `esc` is pressed.
typedef enum : NSUInteger {
    TMMenuPopoverBehaviorApplicationDefined = 0,
    TMMenuPopoverBehaviorTransient = 1,
    TMMenuPopoverBehaviorSemiTransient = 2
} TMMenuPopoverBehavior;

typedef void (^TMMenuPopoverDelegateBlock)(TMMenuPopover *popover);
...
// management. `TMMenuPopover` does not retain itself and when it deallocates it
...
@interface TMMenuPopover : NSResponder
...
// of the popover.
// Note that this must be a subclass of `TMMenuPopoverBackgroundView`.
...
@property (nonatomic, readonly, strong) TMMenuPopoverBackgroundView *backgroundView;
...
// closing the popover.
// See the definition of `TMMenuPopoverBehavior` for more
// information.
@property (nonatomic) TMMenuPopoverBehavior behavior;

// Whether the popover is currently visible.
@property (nonatomic, readonly, getter = isShown) BOOL shown;

// Called before the popover is closed.
@property (nonatomic, copy) TMMenuPopoverDelegateBlock willCloseBlock;
...
@property (nonatomic, copy) TMMenuPopoverDelegateBlock didCloseBlock;

// Called before the popover is opened.
@property (nonatomic, copy) TMMenuPopoverDelegateBlock willShowBlock;

// Called after the block has opened.
// Note that if the open is animated this block will be called _after_ the
// animation has successfully completed.
@property (nonatomic, copy) TMMenuPopoverDelegateBlock didShowBlock;
...
@interface TMMenuPopoverBackgroundView : NSView
...
//                    on.
//
// Returns a newly initialised instance of `TMMenuPopoverBackgroundView`.
