...
+ (void)printViews:(NSView *)containerView j:(int)j {
    for (id c in containerView.subviews) {
        ...
        MTLog(@"%@ %@", lol, NSStringFromClass([c class]));
...
+ (void)printMethods:(id)object {
    ...
    while (currentClass) {
        ...
        unsigned int i = 0;
        for (; i < methodCount; i++) {
            MTLog(@"%@ - %@", [NSString stringWithCString:class_getName(currentClass) encoding:NSUTF8StringEncoding], [NSString stringWithCString:sel_getName(method_getName(methodList[i])) encoding:NSUTF8StringEncoding]);
        }
