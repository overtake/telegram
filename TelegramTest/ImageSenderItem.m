...
-(void)performRequest {
    ...
    [self.uploadOperation setUploadComplete:^(UploadOperation *operation, id input) {
        ...
        if(strongSelf.conversation.type == DialogTypeBroadcast) {
            ...
            request = [TLAPI_messages_sendMedia createWithFlags:strongSelf.message.reply_to_msg_id != 0 ? 1 : 0 peer:strongSelf.conversation.inputPeer reply_to_msg_id:strongSelf.message.reply_to_msg_id media:media random_id:strongSelf.message.randomId  reply_markup:[TL_replyKeyboardMarkup createWithRows:[@[]mutableCopy]]];
        }
