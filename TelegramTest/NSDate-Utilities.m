...
-(NSString *)fuzzyTime {
    ...
    if(days >= 365){
        ...
        formatted = [NSString stringWithFormat:@"last seen %f %@ ago", years, period];
    } else if(days < 365 && days >= 30) {
        float months = round(days / 30) / 2.0f;
        period = (months > 1) ? @"months" : @"month";
        formatted = [NSString stringWithFormat:@"last seen %d %@ ago", (int) ceil(months), period];
    } else if(days < 30 && days >= 2) {
        period = @"days";
        formatted = [NSString stringWithFormat:@"last seen %ld %@ ago", (long)days, period];
    } else if(days == 1){
        period = @"day";
        formatted = [NSString stringWithFormat:@"last seen %ld %@ ago", (long)days, period];
    } else if(days < 1 && minutes > 60) {
        period = (hours > 1) ? @"hours" : @"hour";
        formatted = [NSString stringWithFormat:@"last seen %li %@ ago", (long)hours, period];
    } else {
        period = (minutes < 60 && minutes > 1) ? @"minutes" : @"minute";
        formatted = [NSString stringWithFormat:@"last seen %li %@ ago", (long)minutes, period];
        if(minutes < 1){
            formatted = @"last seen a moment ago";
        }
    }
    return formatted;
}
