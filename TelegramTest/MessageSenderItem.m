...
-(void)performRequest {
    ...
    if(self.conversation.type != DialogTypeBroadcast) {
        ...
        request = [TLAPI_messages_sendMessage createWithFlags:flags peer:[self.conversation inputPeer] reply_to_msg_id:self.message.reply_to_msg_id message:[self.message message] random_id:[self.message randomId] reply_markup:[TL_replyKeyboardMarkup createWithRows:[@[]mutableCopy]]];
    } else {
