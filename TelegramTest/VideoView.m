...
- (id) config
{
	...
	rerror = CVPixelBufferCreateWithBytes( kCFAllocatorDefault, image_width, image_height, pixelFormat, image_data, image_width*image_bytes, NULL, NULL, NULL, &currentFrameBuffer);
	if(rerror != kCVReturnSuccess) {
		NSLog(@"Failed to create Pixel Buffer(%d)", rerror);
		return nil;
	}
	
	rerror = CVOpenGLTextureCacheCreate(kCFAllocatorDefault, 0, [glContext CGLContextObj], [[glContext pixelFormat] CGLPixelFormatObj], 0, &textureCache);
	if(rerror != kCVReturnSuccess) {
		NSLog(@"Failed to create OpenGL texture Cache(%d)", rerror);
		return nil;
	}
	
	rerror = CVOpenGLTextureCacheCreateTextureFromImage(	kCFAllocatorDefault, textureCache, currentFrameBuffer, 0, &texture);
	if(rerror != kCVReturnSuccess) {
