...
-(void)remoteRequest:(BOOL)next peer_id:(int)peer_id callback:(void (^)(NSArray *, ChatHistoryState))callback {
    ...
    self.request = [RPCRequest sendRequest:[TLAPI_messages_search createWithFlags:0 peer:[self.conversation inputPeer] q:@"" filter:[TL_inputMessagesFilterDocument create] min_date:0 max_date:0 offset:0 max_id:self.min_id  limit:(int)self.controller.selectLimit] successHandler:^(RPCRequest *request, TL_messages_messages *response) {
