...
+ (NSString *) mediaMessage:(TLMessage *)message {
    ...
    if([message.media isKindOfClass:[TL_messageMediaPhoto class]]) {
        ...
        if([message.media.document.mime_type isEqualToString:@"video/mp4"] && [message.media.document attributeWithClass:[TL_documentAttributeAnimated class]] != nil) {
            return @"GIF";
        } else if([message.media.bot_result.type isEqualToString:@"gif"] && [message.media.bot_result.send_message isKindOfClass:[TL_botContextMessageMediaAuto class]]) {
            return @"GIF";
        } else if([message.media.bot_result.type isEqualToString:@"photo"] && [message.media.bot_result.send_message isKindOfClass:[TL_botContextMessageMediaAuto class]]) {
            return  NSLocalizedString(@"ChatMedia.Photo", nil);
        }
