   ...
   //     assert(response.dialogs.count == response.messages.count);
        ...
        [response.dialogs enumerateObjectsUsingBlock:^(TL_dialogChannel *channel, NSUInteger idx, BOOL *stop) {
            ...
            if(f.count == 2) {
                ...
                TGMessageGroupHole *groupHole = [[TGMessageGroupHole alloc] initWithUniqueId:-rand_int() peer_id:topMsg.peer_id min_id:minMsg.n_id max_id:topMsg.n_id date:topMsg.date+1 count:0];
